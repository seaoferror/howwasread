...
func NewConsumer(s *service.Service) *Consumer {
	consumerGroup, err := connectConsumer("persist_message")
	if err != nil {
...
func (c *Consumer) distinguishMessage(ctx context.Context, message *sarama.ConsumerMessage) error {
	if message.Topic == "manage_message.prepared" {
		...
		err = c.service.PersistMessage(ctx, uuid.UUID(p.Id), p.ToIds, uuid.UUID(p.RoomId), uuid.UUID(p.FromId), p.ContentType, p.Contents)
		if err != nil {
			return err
		}
		return nil

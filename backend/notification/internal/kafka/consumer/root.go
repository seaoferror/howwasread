...
func NewConsumer(s *service.Service) *Consumer {
	consumerGroup, err := connectConsumer("notify_message")
	if err != nil {
...
func connectConsumer(groupID string) (sarama.ConsumerGroup, error) {
	...
	cfg.ClientID = "consumer.notify_message" + id.String()
...
func (c *Consumer) distinguishMessage(ctx context.Context, message *sarama.ConsumerMessage) error {
	if message.Topic == "notify_message" {
		...
		err = c.service.NotifyMessaging(ctx, p.ToIds, uuid.UUID(p.RoomId), uuid.UUID(p.FromId), p.ContentType, p.Contents)
		if err != nil {
			return err
		}
		return nil
